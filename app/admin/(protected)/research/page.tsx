...
export default async function ResearchAdmin({ searchParams }: PageProps<"/admin/research">) {
  ...
  const data = await getAdminContentService().research(edit);
  return <ResearchManager {...data} notice={Array.isArray(query.notice) ? query.notice[0] : query.notice} editorOpen={creating || Boolean(data.selected)} />;
}
