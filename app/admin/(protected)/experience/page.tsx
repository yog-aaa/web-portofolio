...
export default async function ExperienceAdmin({ searchParams }: PageProps<"/admin/experience">) {
  ...
  const data = await getAdminContentService().experiences(edit);
  return <ExperienceManager {...data} notice={Array.isArray(query.notice) ? query.notice[0] : query.notice} editorOpen={creating || Boolean(data.selected)} />;
}
